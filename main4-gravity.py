...
def init():
    ...
    r.img_center = (r.img.get_rect()[2] // 2, r.img.get_rect()[3])
...
def run():
    ...
    sx, sy = 0, 0
    jsy = 0
    jump_y, jump_y_fade = 20, 10
    rm = 1
    tp = pygame.time.get_ticks() - 1
    ...
    while True:
        ...
        elif(keys[pg.K_DOWN]): csy = 10 * dt
        elif(keys[pg.K_SPACE] and not(jsy > 0)): jsy = jump_y
        else: csy = G * dt
        sx = min(MAX_SPEED, max(-MAX_SPEED, sx + csx))
        sy = min(MAX_SPEED, max(-MAX_SPEED, sy + csy))
        px += sx
        py += sy - jsy
        if(not csx):
          if(abs(sx) > 1):
        ...
        s.blit(r.img, (px + r.img_center[0], py - r.img_center[1]))
        # Рисовать круг
